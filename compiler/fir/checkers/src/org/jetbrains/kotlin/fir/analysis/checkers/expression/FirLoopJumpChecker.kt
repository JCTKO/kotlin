...
object FirBreakOrContinueJumpsAcrossFunctionBoundaryChecker : FirLoopJumpChecker() {
    override fun check(expression: FirLoopJump, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        fun findPathAndCheck(element: FirElement?): Boolean {
            fun findPathAndCheck(element: FirElement, checkElement: FirElement?): Boolean {
            ...
            when (element) {
                ...
                is FirFunction -> {
                    if (findPathAndCheck(element, element.body)) {
                        return true
                    }
                ...
                is FirAnonymousInitializer -> return findPathAndCheck(element, element.body)
                is FirAnonymousObjectExpression -> return findPathAndCheck(element, element.anonymousObject)
                is FirAnonymousFunctionExpression -> return findPathAndCheck(element, element.anonymousFunction)
            }
